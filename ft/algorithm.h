...
public:
  ...
  int unicodeChar() const { return m_char; }

  int charIndex() const { return m_pos - m_begin; }

  unsigned int glyphIndex() { return m_face.cache().getGlyphIndex(m_face, unicodeChar()); }
...
template<typename FaceFT, typename Shaper = HBShaper<FaceFT>>
class ForEachGlyph {
public:
...
private:
  void prepareGlyph()
  {
    ...
    if (m_useKerning && m_prevGlyph && glyphIndex) {
      FT_Vector kerning;
      FT_Get_Kerning(m_face, m_prevGlyph, glyphIndex, FT_KERNING_DEFAULT, &kerning);
      m_x += kerning.x / 64.0;
    }
    ...
    if (m_glyph) {
      ...
      m_glyph->x = m_x + m_glyph->bearingX;
      m_glyph->y = m_y + m_face.height() + m_face.descender() // descender is negative
                   - m_glyph->bearingY;
...
gfx::Rect calc_text_bounds(FaceFT& face, const std::string& str)
{
  ...
  while (feg.next()) {
    if (auto glyph = feg.glyph())
      bounds |= gfx::Rect(int(glyph->x), int(glyph->y), glyph->bitmap->width, glyph->bitmap->rows);
  }
  return bounds;
}
