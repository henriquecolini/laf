...
struct MenuItemInfo {
  ...
  explicit MenuItemInfo(const Type type = Normal, const Action action = UserDefined)
    : type(type)
    , action(action)
  {
  }

  explicit MenuItemInfo(const std::string& text, const Action action = UserDefined)
    : type(Normal)
    , action(action)
    , text(text)
  {
