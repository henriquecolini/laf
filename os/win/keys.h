...
class VkToUnicode {
public:
  VkToUnicode() : m_size(0) { m_ok = (GetKeyboardState(&m_keystate[0]) ? true : false); }
  ...
  void toUnicode(int vk, int scancode)
  {
    ...
    // character.
    m_size =
      ToUnicode(vk, scancode, m_keystate, m_buffer, sizeof(m_buffer) / sizeof(m_buffer[0]), 0);
  ...
  bool isDeadKey() const { return (m_size == -1); }
